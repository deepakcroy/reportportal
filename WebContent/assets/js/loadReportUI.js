function prepareRptParams(){
    var oReportSnippet = "";
    var _ReportName = $("#anetDynamicFrm").attr("data-rptname");
    $(".anet-reportparam").each(function (i) {
        if ($(this).hasClass("anet-checklist")) {
            var str = "";
            $(".anet-checklist-items input[type='checkbox']").each(function () { if ($(this).is(":checked")) { str += $(this).attr('data-val') + ","; } });
        ...
        else if($(this).hasClass("anet-shuttle"))
    	{
        	var optVals="";
    ...
    oReportSnippet = "{" + "\"reportName" + "\":" + "\"" + $("#anetDynamicFrm").attr("data-val") + "\"," + "\"reportTitle" + "\":" + "\"" + $("#hRptTitle").text() + "\"," + "\"reportFile" + "\":" + "\"" + _ReportName + "\"," + "\"params" + "\": [" + oReportSnippet + "]}";
...
function prepareReportUrl() {
    ...
    $(".anet-codeblock").each(function (i) {
        eval(trim($(this).text()));
    });
    ...
    if (arrRst[0] != "")//Procedure
        ...
        _sqlProcedure = $("#anetDynamicFrm").attr("data-procedure");

    if (arrRst[1] != "")//Report
        _ReportName = arrRst[1];
    else
        _ReportName = $("#anetDynamicFrm").attr("data-rptname");
    ...
    // checkbox
    var jsonObj = "";
    $(".anet-checkbox:input[type=checkbox]").each(function (i) {
    ...
    //anet-conditionltextbox
    $(".anet-conditionltextbox").each(function (i) {
        if ($(this).attr("data-param") != null) {
    ...
    // anet-datepicker
    $(".anet-datepicker").each(function (i) {
        if ($(this).attr("data-param") != null)
            jsonObj += "{" + "\"Name" + "\":" + "\"" + $(this).attr("data-param") + "\"," + "\"Value" + "\":" + "\"" + $(this).val() + "\"},"
    });
    // anet-monthpicker
    $(".anet-monthpicker").each(function (i) {
        if ($(this).attr("data-param") != null) {
    ...
    // anet-quarterpicker
    $(".anet-quarterpicker").each(function (i) {
    ...
    // anet-DropDown
    $(".anet-DropDown").each(function (i) {
        if ($(this).attr("data-param") != null)
            jsonObj += "{" + "\"Name" + "\":" + "\"" + $(this).attr("data-param") + "\"," + "\"Value" + "\":" + "\"" + $(this).val() + "\"},"
    });
    // anet-fiscalyear
    $(".anet-fiscalyear").each(function (i) {
        if ($(this).attr("data-param") != null) {
    ...
    // anet-radio
    $(".anet-radio:radio:checked").each(function (i) {
        if ($(this).attr("data-param") != null)
            jsonObj += "{" + "\"Name" + "\":" + "\"" + $(this).attr("data-param") + "\"," + "\"Value" + "\":" + "\"" + $(this).attr("data-val") + "\"},"
    });
    //anet-num-range
    $(".anet-num-range").each(function (i) {
        if ($(this).attr("data-param") != null) {
    ...
    // anet-dataparam
    $(".anet-dataparam").each(function (i) {
        if ($(this).attr("data-param") != null)
            jsonObj += "{" + "\"Name" + "\":" + "\"" + $(this).attr("data-param") + "\"," + "\"Value" + "\":" + "\"" + $(this).attr("data-val") + "\"},"
    });
    //User_Id
    jsonObj += "{" + "\"Name" + "\":" + "\" user_id \"," + "\"Value" + "\":" + "\"" + $("#loggedUser").attr("data-networkalias") + "\"},";
    //$("#anetDynamicFrm").attr("data-procedure")
    ...
    rdlReportObj = "{" + "\"ReportNumber" + "\":" + "\"" + $("#anetDynamicFrm").attr("data-val") + "\"," + "\"ReportTitle" + "\":" + "\"" + $("#hRptTitle").text() + "\"," + "\"RDLName" + "\":" + "\"" + _ReportName + "\"," + "\"Params" + "\": [" + rdlReportObj + "]}";
...
function exportReportAS(obj) {
    if ($("#menuExportAs").length > 0) {
        ...
        var str = "";
        str = "<div id='menuExportAs' style='min-width:150px; height:60px; border:1px solid #ccc; border-radius:5px; padding:5px; position:absolute;z-index:100; margin-top:-99px; margin-left:303px; background-color:#fff; '><ul id='anet-exportas-menu'><li onclick='exportReport(this)'><a href='#'><span class='menu-ico-excel'></span>Excel (Data)</a></li><li onclick='exportReport(this)'><a href='#'><span class='menu-ico-excelf'></span>Excel (Formatted)</a></li><li onclick='exportReport(this)'><a href='#'><span class='menu-ico-pdf'></span>PDF</a></li></ul></div>";
        $(str).insertAfter(obj);
    }
}
...
function resetCurrentUI() {
    ...
    //Specially Handel shuttle
    $(".anet-shuttle").each(function () {
    	var attrName = $(this).attr('name');
        if (typeof attrName !== typeof undefined && attrName !== false) {
...
function prepareJobScheduleUrl() {
    ...
    $(".anet-shuttle").each(function (i) {
        if ($(this).attr("name") != "") {
            ...
            dlbQStr = dlbQStr + dlbVals.substring(0, dlbVals.length - 1) + "&";
            if ($(this).attr("data-param") != null)
                jsonObj += "{" + "\"Id" + "\":" + "\"" + $(this).attr("Id") + "\"," + "\"Type" + "\":" + "\"Shuttle\"," + "\"Name" + "\":" + "\"" + $(this).attr("data-param") + "\"," + "\"Value" + "\":" + "\"" + dlbVals + "\"},"
        }
    });
    ...
    // anet-sch-datepicker
    $(".anet-sch-datepicker").each(function (i) {
        if ($(this).attr("data-param") != null)
    ...
    $(".anet-DropDown").each(function (i) {
        if ($(this).attr("data-param") != null)
            jsonObj += "{" + "\"Id" + "\":" + "\"" + $(this).attr("Id") + "\"," + "\"Type" + "\":" + "\"DropDown\"," + "\"Name" + "\":" + "\"" + $(this).attr("data-param") + "\"," + "\"Value" + "\":" + "\"" + $(this).val() + "\"},"
    });
    // anet-fiscalyear
    $(".anet-fiscalyear").each(function (i) {
        if ($(this).attr("data-param") != null) {
...
function showCurrentReportJobLog(obj) {
    $("#joblog_dialog").dialog({
        modal: true,
        resizable: true,
        title: "Logs of Report " + $("#anetDynamicFrm").attr("data-val"),
        width: 850,
        height: 500,
        close: function () {
        },
...
function showJobDetail(obj, exeid) {
    ...
    $("#jobdetail_dialog").dialog({
        modal: true,
        resizable: true,
        title: "Job Setting of Report " + $("#anetDynamicFrm").attr("data-val"),
        width: 650,
        height: 300,
        close: function () {
        },
...
function getJobExecutions(obj) {
    ...
    if ($(obj).attr("data-jobmode") == "false") {
        ...
        $(".frmContent,.anet-datarow").css("background-color", "#C4E0B6");
        // $(".data-row").css("background-color", "#B4FBB4");

        $(obj).attr("src", "Images/Theme1/arrow-left.png");
        $("#anet-rpt-ctrls .anet-submit").hide();
        //$("#anet-rpt-ctrls .anet-submit").not("#btnLoadDefault").hide();
        $("#anet-rpt-ctrls").append("<input id='btnRCVerify' class='anet-job-sch-btn' onclick='verifyProcedure(this)' value='Verify Report' type='button'/>");
        $(".anet-datepicker").each(function () {
            var _parent = $(this).parent();
            $(this).parent().find('.anet-datepicker,.anet-datepicker-btn').hide();
            $(_parent).append("<select id='" + $(this).attr("Id") + "_jobsch' class='anet-sch-datepicker' data-param='" + $(this).attr('data-param') + "'><option value='CurrentDate'>Current Date</option><option value='DayBeforeToDay'>Day Before Today</option><option value='FirstMonthDate'>First Month Date</option><option value='LastMonthDate'>Last Month Date</option><option value='DayBeforeLastMonthDate'>Day Before Last Month Date</option><option value='FiscalYearStartDate'>Fiscal Year Start Date</option><option value='FiscalYearEndDate'>Fiscal Year End Date</option></select>");
        });
        $(".anet-monthpicker").each(function () {
            var _parent = $(this).parent();
            $(this).parent().find('.anet-monthpicker,.anet-monthpicker-btn').hide();
            $(_parent).append("<select id='" + $(this).attr("Id") + "_jobsch' class='anet-sch-monthpicker' data-param='" + $(this).attr('data-param') + "'><option value='CurrentDate'>Current Date</option><option value='DayBeforeToDay'>Day Before Today</option><option value='FirstMonthDate'>First Month Date</option><option value='LastMonthDate'>Last Month Date</option><option value='DayBeforeLastMonthDate'>Day Before Last Month Date</option><option value='FiscalYearStartDate'>Fiscal Year Start Date</option><option value='FiscalYearEndDate'>Fiscal Year End Date</option></select>");
        });
        $("#frmContentInner").append("<div id='anet-sch-joblist' style='display:none;'><img id='introLoader' style='display:none;' src='/Images/Theme1/loading35.gif'><div id='anet-sch-joblist-loader'></div><div id='anet-sch-joblist-detail'></div></div>");
        ...
        $(".frmContent,.anet-datarow").css("background-color", "#EDEFED");

        $("#anet-rpt-ctrls .anet-job-sch-btn").remove();
        $("#anet-rpt-ctrls .anet-submit").show();

        $(".anet-sch-datepicker").remove();
        $(".anet-datepicker,.anet-datepicker-btn").show();

        $("#anet-sch-joblist").remove();
    }
}
function loadJobExecutions() {
    var qStr = "rptNumber=" + trim($("#anetDynamicFrm").attr("data-val"));
    $.ajax({
        type: 'GET',
        ...
        beforeSend: function () {
            $("#introLoader").show();
            //$("#anet-sch-joblist-loader").html('');
        },
        complete: function () {
            $("#introLoader").hide();
        },
        success: function (result) {
            $("#anet-sch-joblist-detail").html(result);
        },
...
function verifyProcedure(obj) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#anet-sch-joblist-loader").html(result);
        },
        error: function (xhr, ajaxOptions, thrownError) {
            alert(xhr.status + ":" + thrownError);
...
function showReport(e,obj) {
    ...
    var _report = $("#anetDynamicFrm").attr("data-val");
    var _dt = new Date();
    $("#hdnReportExecutionQStr").val(qStr);
    _report = $("#anetDynamicFrm").attr("data-rptname");
...
function fillReportControl(ctrlId, ctrlType, ctrlData) {
    ...
    switch (ctrlType) {
        case "DropDown":
        	if($("#"+ctrlId).hasClass("anet-dropdown"))
        		$("#" + ctrlId).html(decoded);
            break;
        case "ListBox":
        	if($("#"+ctrlId).hasClass("anet-listbox"))
        		$("#" + ctrlId).html(decoded);
            break;
        case "Shuttle":
        	if($("#"+ctrlId).hasClass("anet-shuttle"))
